function [train_data_s,test_img] = data_process_train(train_data,T11L,T12L,T13L,T22L,T23L,T33L,nwin)
%*****************************************************************
...
for i=1:3:row-nwin                % step:3
    for j=1:3:col-nwin
       test_img(:,:,1,k) = T11_all(i:i+nwin-1,j:j+nwin-1);
       test_img(:,:,2,k) = T12_all(i:i+nwin-1,j:j+nwin-1);
       test_img(:,:,3,k) = T13_all(i:i+nwin-1,j:j+nwin-1); 
       test_img(:,:,4,k) = T22_all(i:i+nwin-1,j:j+nwin-1);
       test_img(:,:,5,k) = T23_all(i:i+nwin-1,j:j+nwin-1);
       test_img(:,:,6,k) = T33_all(i:i+nwin-1,j:j+nwin-1);
       k=k+1;
    end
end
